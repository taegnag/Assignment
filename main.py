...
class Property(Square):
    ...
    # modification 2 start here // 수정 : 기존 40칸에서 부루마불에서 사용하지 않는 칸 삭제, 칸 수 및 보드 위치 수정
...
# modification 4 start here 수정 : turnNum(player의 턴 수) 변수 추가, 부루마불에서 사용하지 않는 trade, auction 관련 기능 삭제
...
# modification 4 end here
    ...
    # modification 5 start here 수정 : 줄인 칸 수 기준으로 말이 표시되는 위치 수정
    def getPos(self): 
        if 0 <= self.boardpos < 9:
...
class AI: # There's an Eve object of the Player class and an AI object of the AI class. I like to think of 'Eve: Player' as Eve's body and 'AI: AI' as her brain.
    ...
    # modification 6 start here 수정 : 직관적으로 AI의 자금이 40% 이하가 되기 전까지 모든 부동산을 구매하도록 변경
    def wantsProp(self, prop): # Returns boolean of whether she wants to buy a given property or not.
        if prop.getPrice() <= Eve.money * 0.6:
            return True
        return False 
    # modification 6 end here
    
    # modification 7 start here
    # 기존의 같은 색의 부동산을 모두 구매한 뒤 같은 색의 부동산 간의 집 개수를 동일하게 늘려야 했던 것과는 달리
    # 부루마불의 규칙으로 본인이 소유한 부동산을 밟을 때마다 집 개수를 늘릴 수 있도록 변경
    def develop(self, current): # Returns the next property she wants to build a house on.
        ...
        if buildingProp.houses == 0 and self.player.money * 0.6 >= buildingProp.getCostOfHouse():
            ...
            return None
    # modification 7 end here
    
        
    # modification 8 start here 
    # 부동산을 매각한 뒤 빚만 갚으면 다시 부동산 소유 권한을 돌려받는 모노폴리 규칙과는 달리
    # 부동산을 매각하면 소유 권한을 잃어버리는 부루마불 규칙으로 변경
...
# -------------------------------------------------------------------------------
# ALERT CLASSES
# modification 9 start here 우주여행 칸 알림 추가
class Alert:
...
while not (winner==Eve or winner==user):
    ...
    if user.isTurn and (beginning or not((type(alert) == EveAlert) or alert.heading == 'Memories from 2008')):
            ...
            if user.timeMoving == 0:
                ...
                if type(currentSquare) == Property:
                    ...
                    if not currentSquare.paid:
                        ...
                        currentSquare.paid = True
                        # modified 12 end here
                        
        # modified 13 start here
        ...
        if Eve.inJail and alert.heading != 'Stayin\' Alive' and alert.heading != 'Another one bytes the dust' and alert.heading != 'Developing countries':
            ...
            Eve.boardpos = 9
            if Eve.jailTurns == 3:
                    alert = EveAlert('Escaping CAPTCHA', 'Eve was in jail for 3 turns.')
            elif Eve.jailTurns == 0:
                if AI.useGojf():
                    Eve.getOutOfJailFreeCards.remove(Eve.getOutOfJailFreeCards[0])
                    Eve.inJail = False
                    alert = EveAlert('Dodgy Dealing', "Eve did some dodgy deals")
                else:
                    Eve.injail = True   
